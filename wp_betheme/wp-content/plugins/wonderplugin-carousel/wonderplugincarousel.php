...
define('WONDERPLUGIN_CAROUSEL_VERSION', '14.8');
...
define('WONDERPLUGIN_CAROUSEL_PLUGIN_VERSION', '14.8');
...
class WonderPlugin_Carousel_Plugin {
	...
	public function init() {
		...
		$jetpackdisablelazy = get_option( 'wonderplugin_carousel_jetpackdisablelazyload', 1 );
		if ($jetpackdisablelazy == 1)
		{
			add_filter( 'jetpack_lazy_images_blacklisted_classes', array($this, 'modify_jetpack_carousel_lazy_classes'), 10, 3 );
		}
	}
	...
	function modify_jetpack_carousel_lazy_classes( $classes ) {
		
		if (empty( $classes ))
			$classes = array();
		$classes[] = "amazingcarousel-image-img";
		return $classes;
	}
